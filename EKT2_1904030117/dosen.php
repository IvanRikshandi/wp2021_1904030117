...
$datdos = query("SELECT * FROM data_dosen");

if (isset($_GET['search'])) {
  $datdos = search($_GET['keyword']);
}
...
<body>
  ...
  <div class="row mt-4">
    ...
    <div class="col-md-10 p-5 pt-5">
      <!-- konten -->
      <h3><i class="fas fa-users"></i> Daftar Dosen Fakultas Teknik</h3>
      <hr>

      <!-- Pencarian Data -->
      <form action="dosen.php" method="get">
        <input type="text" id="keyword" size="50" placeholder="Masukan Keyword" autocomplete="off">
        <button type="submit" class="btn btn-primary" name="search" role="button"> CARI</button>
      </form>
      ...
      <table class="table table-striped">
        ...
        <tbody>
          <?php $no = 1; ?>
          <?php foreach ($datdos as $ddos) : ?>
            <tr class="text-center">
              <th scope="row"><?php echo $no; ?></th>
              <td><?php echo $ddos['nama']; ?></td>
              <td><img src="image/<?php echo $ddos['foto_dosen']; ?>" width="75px"></td>
              <td><a href="detaildosen.php?id=<?= $ddos['id']; ?>" class="btn btn-warning" role="button">Detail</a></td>
            </tr>
            <?php $no++ ?>
          <?php endforeach ?>
        </tbody>
